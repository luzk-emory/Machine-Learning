...
for i in lower_case_documents:
    ...
    sans_punctuation_documents.append(i.translate(translation))
...
X_train, X_test, y_train, y_test = train_test_split(df['sms_message'], 
                                                    df['label'], 
                                                    random_state=1) # default size is 0.25 for test set
...
training_data2 = count_vector.transform(X_train)
testing_data2  = count_vector.transform(X_test)
print(training_data2.shape)
print(testing_data2.shape)
aaa = (testing_data-testing_data2).todense()
print(type(aaa))
import numpy as np
np.sum(aaa)
...
# P(~D|Pos)
p_no_diabetes_pos = p_pos_no_diabetes*p_no_diabetes/p_pos # TODO
...
Solution
'''
p_j_fi = p_j_text/(p_j_text+p_g_text)# TODO
...
Solution
'''
p_g_fi = p_g_text/(p_j_text+p_g_text)# TODO
...
training_data[:5,:10].toarray() # already in matrix form; each row is a keywards frequency vector
# y_train.head()
